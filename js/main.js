...
// Show a help message about the autosampling feature
function showAutosamplerHelpMessage() {
    setTimeout(() => {
        console.log("Showing autosampler help message");
        const helpMessage = document.createElement('div');
        helpMessage.id = 'autosampler-help';
        helpMessage.innerHTML = `
            <h3>NEW! Autosampler Feature</h3>
            <p>Press <strong>R</strong> while hovering over a puck or connection line to activate autosampling.</p>
            <p>Creates chopped samples from connected pucks for hip-hop style sampling.</p>
            <p>Press <strong>R</strong> again to stop.</p>
        ...
        if (closeButton) {
            ...
            closeButton.addEventListener('click', () => {
                helpMessage.style.animation = 'fadeOut 0.5s ease-out';
                setTimeout(() => {
                    helpMessage.remove();
                }, 480);
            });
        ...
        setTimeout(() => {
            if (document.getElementById('autosampler-help')) {
                helpMessage.style.animation = 'fadeOut 0.5s ease-out';
                setTimeout(() => {
                    if (document.getElementById('autosampler-help')) {
                        helpMessage.remove();
...
// Call after the window loads
window.addEventListener('load', () => {
    showAutosamplerHelpMessage();
});